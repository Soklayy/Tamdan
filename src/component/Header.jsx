...
const Header = () => {
    ...
    return (
        <>
            <div className={`fixed top-0 md:hidden z-20 h-screen w-screen bg-green-500 bg-opacity-25 duration-500 ${openMenu ? 'left-0' : '-left-full'}`}>
                <div className="relative z-20">
                    <nav className="absolute bg-gray-800 w-1/2 h-screen flex flex-col  md:hidden gap-3 py-4 px-5 text-white capitalize">
                        ...
                        {Menu.map((menu, key) => (
                            <NavLink
                                to={menu.url}
                                className={`hover:text-green-500 cursor-pointer font-semibold transition-colors duration-300 active:`}
                                key={key} >
                                {menu.title}
                            </NavLink>
                        ))}
            ...
            <header className="flex justify-between items-center bg-gray-900 shadow shadow-slate-800 px-10 py-2 lg:px-40 h-16 w-full sticky top-0 z-10">
                ...
                <nav className="items-center hidden space-x-8 md:flex text-white capitalize">
                    {Menu.map((menu, key) => (
                        <NavLink
                            to={menu.url}
                            className="hover:text-green-500 cursor-pointer font-semibold duration-300 active:"
                            key={key}>
                            {menu.title}
                        </NavLink>
                    ))}
                ...
                <div className="flex gap-5 items-center  text-white">
                    ...
                    <Link className="relative w-8 h-8 cursor-pointer" to={'/chat'}>
                        <span className="absolute -top-2 -right-2">
                            <span className="flex h-2 w-2 items-center justify-center rounded-full bg-red-500 p-3 text-xs text-white">3</span>
                        </span>
